...
defmodule Day9.Part1 do
  ...
  represent groups - sequences that begin with `{` and end with `}`. Within a group,
  ...
  * `{{<a!>},{<a!>},{<a!>},{<ab>}}`, score of 1 + 2 = 3.
  ...
  ## Examples:
    ...
    iex> Part1.solve("{{{},{},{{}}}}")
    16
    iex> Part1.solve("{<a>,<a>,<a>,<a>}")
    1
    iex> Part1.solve("{{<ab>},{<ab>},{<ab>},{<ab>}}")
    9
    iex> Part1.solve("{{<!!>},{<!!>},{<!!>},{<!!>}}")
    9
    iex> Part1.solve("{{<a!>},{<a!>},{<a!>},{<ab>}}")
    3
  """
  def solve(input) do
    input
    |> String.graphemes
    |> Day9.remove_garbage
    |> Enum.reduce(%{level: 0, value: 0}, fn(c, %{level: level, value: value} = acc) ->
      # The list of valid graphemes is reversed here - we are iterating from right to left
      cond do
        c == "}" -> %{acc | level: level + 1, value: value + level + 1}
        c == "{" -> %{acc | level: level - 1}
        true -> acc
      end
    end)
