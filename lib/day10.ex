defmodule Day10 do
  ...
  the marks within it. After doing this many times, the order of the marks
  ...
  begins at 0 (the first element in the list), a skip size (which starts at 0), and
  ...
  * Increase the skip size by one.
  ...
  def knot_hash(len, %{list: list, current_pos: current_pos, skip: skip}) do
    reversed = take_reversed_sublist(list, current_pos, len)
    updated = update_with_reversed_sublist(list, reversed, current_pos)
    new_pos = rem(current_pos + len + skip, length(list))

    %{list: updated, current_pos: new_pos, skip: skip + 1}
  end

  defp take_reversed_sublist(list, start, len) do
    Enum.reduce(0..(len - 1), [], fn(i, acc) ->
      [Enum.fetch!(list, rem(start + i, length(list))) | acc]
    end)
  end

  defp update_with_reversed_sublist(list, reversed, start) do
    Enum.reduce(Enum.with_index(reversed), list, fn({value, index}, acc) ->
      List.replace_at(acc, rem(start + index, length(list)), value)
    end)
  end
end
...
defmodule Day10.Part1 do
  ...
  ## Examples:
    iex> Part1.solve("3,4,1,5", 0..4)
    12
  """
  def solve(input, range \\ 0..255) do
    input
    |> String.split(",", trim: true)
    |> Enum.map(fn c -> String.to_integer(c) end)
    |> Enum.reduce(%{list: Enum.to_list(range), current_pos: 0, skip: 0}, fn(len, acc) ->
      Day10.knot_hash(len, acc)
    end)
    |> checksum
  end

  defp checksum(%{list: list}) do
    [first | tail] = list
...
defmodule Day10.Part2 do
  ...
  but as a string of bytes instead. Unless otherwise specified, convert characters
  to bytes using their ASCII codes. This will allow you to handle arbitrary ASCII strings,
  ...
  be preserved between rounds. For example, if the previous example was your first round,
  you would start your second round with the same length sequence
  (`3, 4, 1, 5, 17, 31, 73, 47, 23`, now assuming they came from ASCII codes and include the suffix),
  ...
  the first element in the dense hash is the first sixteen elements of the sparse hash XOR'd together,
  ...
  the final output is the dense hash in hexadecimal notation. Because each number in your
  dense hash will be between 0 and 255 (inclusive), always represent each number as two
  hexadecimal digits (including a leading zero as necessary).
  ...
  Because every Knot Hash is sixteen such numbers, the hexadecimal representation is
  ...
  ## Examples:
    iex> Day10.Part2.solve("")
    "a2582a3a0e66e6e86e3812dcb672a272"
    iex> Day10.Part2.solve("AoC 2017")
    "33efeb34ea91902bb2f59c9920caa6cd"
    iex> Day10.Part2.solve("1,2,3")
    "3efbe78a8d82f29979031a4aa0b16a9d"
    iex> Day10.Part2.solve("1,2,4")
    "63960835bcdc130f0b66d7ff4f6a5a8e"
  """
  def solve(input, range \\ 0..255) do
    length_seq = String.to_charlist(input) ++ :binary.bin_to_list(@magic_binary)

    Enum.reduce(1..64, %{list: Enum.to_list(range), current_pos: 0, skip: 0}, fn(_round, final) ->
      Enum.reduce(length_seq, final, fn(len, acc) ->
        Day10.knot_hash(len, acc)
      end)
    end)
    |> Map.get(:list)
    |> Enum.chunk_every(16)
    |> Enum.map(fn chunk ->
      Enum.reduce(chunk, 0, fn x, acc -> Bitwise.bxor(x, acc) end)
    end)
    |> to_dense_hash
  end

  defp to_dense_hash(chunks) do
    chunks
    |> :binary.list_to_bin
