...
fn part_1(program: Vec<i64>) -> usize {
    ...
    while let Flag::Running = run_program(&mut state) {
        // Keep running
    }

    let output_len = state.output_buffer.len();
    let mut pixel = 0;
    let mut screen: HashMap<(i64, i64), i64> = HashMap::new();

    while pixel < output_len/3 {
        let x = state.output_buffer.pop_front().unwrap();
        let y = state.output_buffer.pop_front().unwrap();
        let tile_id = state.output_buffer.pop_front().unwrap();
    ...
    return screen.iter().filter(|(_coords, tile_id)| **tile_id == 2).count();
}

fn part_2(program: Vec<i64>) -> io::Result<(i64)> {
    let mut memory = program.clone();
    memory.resize(MEMORY_SIZE, 0);
    memory[0] = 2; // 2 = free play
    ...
    let mut state = ProgramState{
        mem: memory,
        input_buffer: VecDeque::from(vec![joystic]),
        ..Default::default()
    };
