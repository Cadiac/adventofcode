...
impl GameOfSeats {
    ...
    fn adjacent_occupied_count(&self, seat: (i32, i32)) -> usize {
        ADJACENT_TILES.iter().filter(|(x, y)| self.is_occupied((seat.0 + x, seat.1 + y))).count()
    }
    ...
    fn visible_occupied_count(&self, seat: (i32, i32)) -> usize {
        ADJACENT_TILES.iter().filter(|direction| self.occupied_at_direction(seat, **direction)).count()
    }
...
fn main() -> () {
    ...
    let part_2_result = GameOfSeats::new(INPUT_FILE).part_2();
