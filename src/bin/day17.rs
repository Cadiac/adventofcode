...
fn part_1(program: Vec<i64>) -> io::Result<i64> {
    ...
    let mut world: HashSet<(i64, i64)> = HashSet::new();
    let mut intersections: HashSet<(i64, i64)> = HashSet::new();
    let mut robot: ((i64, i64), usize) = ((0, 0), 0); // (x, y), facing
    ...
    loop {
        ...
        while let Some(output) = state.output_buffer.pop_front() {
            world_str.push((output as u8) as char);
        }

        for (y, line) in world_str.lines().enumerate() {
            for (x, character) in line.chars().enumerate() {
                let x_i64 = x as i64;
                let y_i64 = y as i64;

                match character {
                    '#' => { world.insert((x_i64, y_i64)); },
                    '^' => { robot = ((x_i64, y_i64), 0); },
                    '>' => { robot = ((x_i64, y_i64), 1); },
                    'v' => { robot = ((x_i64, y_i64), 2); },
                    '<' => { robot = ((x_i64, y_i64), 3); }
                    _ => ()
                }
            }
        ...
        for tile in &world {
            ...
            if world.contains(&(tile_x + 1, *tile_y)) &&
               world.contains(&(tile_x - 1, *tile_y)) &&
               world.contains(&(*tile_x, tile_y + 1)) &&
               world.contains(&(*tile_x, tile_y - 1)) {
                // Has neighbours, mark intersection
                intersections.insert((*tile_x, *tile_y));
            }
        }

        // Print the world
        window.mvprintw(0, 10, format!("[ROBOT]: ({}, {})", (robot.0).0, (robot.0).1));
        window.mvprintw(1, 0, world_str.clone());

        for intersection in &intersections {
            window.mvprintw(intersection.1 as i32 + 1, intersection.0 as i32, "O");
        }

        if Flag::Halted == flag {
            match window.getch() {
                Some(pancurses::Input::Character(x)) if x == 'q' => {
                    endwin();
                    let sum_of_alignment_params = intersections.iter()
                        .fold(0, |sum, (x,y)| sum + x * y );
                        
                    return Ok(sum_of_alignment_params);
                },
                _ => (),
            }
        }
...
// Used to generate list of moves, which was used to figure out the program
fn explore(world: HashSet<(i64, i64)>, robot: ((i64, i64), usize)) -> () {
    let movements = [(0, -1), (1, 0), (0, 1), (-1, 0)];
        let ((mut robot_x, mut robot_y), mut facing) = robot;

        let mut explored: HashSet<(i64, i64)> = HashSet::new();
        let mut moves: Vec<(char, usize)> = Vec::new(); // turn, steps

        // Figure out movements to completely explore the scaffold
        loop {
            explored.insert((robot_x, robot_y));
            // Figure which way to turn
            let turn;

            if (facing == 1 || facing == 3) && world.contains(&(robot_x, robot_y - 1)) && !explored.contains(&(robot_x, robot_y - 1)) {
                turn = if facing == 3 { 'R' } else { 'L' };
                facing = 0;
            } else if (facing == 0 || facing == 2) && world.contains(&(robot_x + 1, robot_y)) && !explored.contains(&(robot_x + 1, robot_y)) {
                turn = if facing == 0 { 'R' } else { 'L' };
                facing = 1;
            } else if (facing == 1 || facing == 3) && world.contains(&(robot_x, robot_y + 1)) && !explored.contains(&(robot_x, robot_y + 1)) {
                turn = if facing == 1 { 'R' } else { 'L' };
                facing = 2;
            } else if (facing == 2 || facing == 0) && world.contains(&(robot_x - 1, robot_y)) && !explored.contains(&(robot_x - 1, robot_y)) {
                turn = if facing == 2 { 'R' } else { 'L' };
                facing = 3;
            } else {
                // We found the end
            ...
            let mut current_move = (turn, 0);

            // While the direction we face has tiles move there
            while world.contains(&(robot_x + movements[facing].0, robot_y + movements[facing].1)) {
                // There is a tile above us, keep moving there;
                robot_x += movements[facing].0;
                robot_y += movements[facing].1;
                current_move.1 += 1;
            }
...
// Collecting list of required moves it was quite easy to manually find the suitable route for part 2
// We first ran explore(), then used the result to construct this program
// MAIN: A,C,C,B,A,C,B,A,C,B
// A: L,6,R,12,L,4,L,6
// B: L,6,L,10,L,10,R,6
// C: R,6,L,6,R,12
// TODO: Do this properly, figure out zip algorithm to do that ^ and do nice UI visualization.
fn part_2(program: Vec<i64>) -> io::Result<i64> {
    let mut memory = program.clone();
    memory.resize(MEMORY_SIZE, 0);
    memory[0] = 2;

    let movement_routine =
        "A,C,C,B,A,C,B,A,C,B\n\
        L,6,R,12,L,4,L,6\n\
        L,6,L,10,L,10,R,6\n\
        R,6,L,6,R,12\n\
    ...
    let ascii_routine: Vec<i64> = movement_routine
        .as_bytes()
        .iter()
    ...
    while let Some(output) = state.output_buffer.pop_front() {
        if output > 255 {
            return Ok(output); // TODO: We terminate here early and don't print anything for part 2
        }
        world_str.push((output as u8) as char);
    }
    for (y, line) in world_str.lines().enumerate() {
    ...
    explore(world.clone(), robot); // Used to construct the route
...
fn main() -> () {
    ...
    let sum = part_1(program.clone()).unwrap();
    let dust = part_2(program.clone()).unwrap();
