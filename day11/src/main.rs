...
fn part_1(program: Vec<i64>) -> HashMap<(i32, i32), i64> {
    let mut memory = program.clone();
    memory.resize(MEMORY_SIZE, 0);

    let mut facing_direction = 0;
    // 0 index is facing up. Inner vec index 0 = left, 1 = right

    let turn_directions: Vec<Vec<(i32, i32)>> = vec![
        vec![(-1, 0), (1, 0)], // Facing up
        vec![(0, -1), (0, 1)], // Facing right
        vec![(1, 0), (-1, 0)], // Facing down
        vec![(0, 1), (0, -1)], // Facing left
    ];
    ...
    let mut picture: HashMap<(i32, i32), i64> = HashMap::new();
    let mut printer_head = (0, 0);

    loop {
        let current_tile_color = picture.entry(printer_head).or_insert(0);
        state.input_buffer.push_back(current_tile_color.clone());

        // println!("[INFO]: Printer at {:?} facing {}, color: {}", printer_head, facing_direction, current_tile_color);

        match run_program(&mut state) {
            Flag::InputRequired => {
                let color = state.output_buffer.pop_front().unwrap();
                let direction = state.output_buffer.pop_front().unwrap();

                // Paint the current position with color
                picture.insert(printer_head, color);

                // Before the printer turns get the instructions on where to turn
                let move_to_direction = turn_directions[facing_direction as usize][direction as usize];
                printer_head.0 += move_to_direction.0;
                printer_head.1 += move_to_direction.1;

                // Update which direction the printer is now facing
                if direction == 0 {
...
fn part_2(program: Vec<i64>) -> String {
    let mut memory = program.clone();
    memory.resize(MEMORY_SIZE, 0);
    ...
    // Enough for this input, maybe change #lazy
    let mut vec_picture: Vec<Vec<i64>> = vec![vec![0; 50]; 10];

    for ((x,y), color) in picture {
        vec_picture[y as usize][x as usize] = color;
    }

    let mut result_picture = String::new();

    for row in vec_picture {
        result_picture += "\n[INFO]: ";
        for column in row {
            if column == 1 {
                result_picture += "█";
...
fn main() -> () {
    ...
    let part1_picture = part_1(program.clone());
    println!("Part 1: {:?}", part1_picture.len());

    let part2_picture = part_2(program.clone());
    println!("Part 2: {}", part2_picture);
}
